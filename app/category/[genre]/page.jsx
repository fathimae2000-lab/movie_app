...
export default function CategoryPage({ params }) {
  const { genre } = use(params); // ✅ Next.js 15 fix
  ...
  return (
    <section className="w-full bg-[#081b27] min-h-screen px-6 md:px-16 py-16 md:pt-40">
      {/* Heading */}
      <div className="text-center mb-12">
        ...
        <p className="text-slate-400 mt-3 text-sm md:text-xl font-light">
          Reviews filtered by category: {genreName}
        </p>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="aspect-[2/3] rounded-xl bg-[#162436] animate-pulse"
            />
          ))}
        </div>
      ) : (
